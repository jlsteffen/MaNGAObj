...
This script prints the extracted and modelled spectra from SPFIT for every 
object identified. The printed spectra is then used to visually classify each
object into the following categories;

    1. good - galaxy type spectra with good signal-to-noise
    2. low_snr - object with poor signal-to-noise such that it can't be 
       identified
    3. border - object on the border between good and low_snr
    4. z_off - galaxy whose redshift is significantly different from the MaNGA
       object's redshift
    5. blagn - Broad-line active galactic nucleus
    6. z_off_qso - high redshift quasar
    7. star - stellar object
    8. others - objects that cannot otherwise be classified
...
# Collect spfit files
files = glob(filepath + '/10.spfit/r1kpc/*_spec.fits')
